...
namespace SmartWeatherBot.Database
{
    ...
    {
        public static IEnumerable<T> Query<T>(this IMainDatabase db, string query, object param = null) where T : class
        {
            try
            {
                if (db.GetNewConnection(true) is IDbConnection connection)
                {
                    using (connection)
                    {
                        ...
                        {
                            var list = connection.Query<T>(query, param);
                            return list;
                        }
                    }
                }
        ...
        public static async Task<IEnumerable<T>> QueryAsync<T>(this IMainDatabase db, string query, object param = null) where T : class
        {
            try
            {
                if (db.GetNewConnection(true) is IDbConnection connection)
                {
                    using (connection)
                    {
                        if (connection.State == ConnectionState.Open)
                        {
                            var list = await connection.QueryAsync<T>(query, param);
                            return list;
                        }
                    }
                }
        ...
        public static bool InsertNoIndex<T>(this IMainDatabase db, T value) where T : class
        {
            try
            {
                if (db.GetNewConnection(true) is IDbConnection connection)
        ...
        public static async Task<bool> InsertNoIndexAsync<T>(this IMainDatabase db, T value) where T : class
        {
            try
            {
                if (db.GetNewConnection(true) is IDbConnection connection)
            ...
            try
            {
                if (db.GetNewConnection(true) is IDbConnection connection)
                {
                    using (connection)
                    {
                        if (connection.State == ConnectionState.Open)
                        {
                            var insert_ind = connection.Insert(data);
                            if (insert_ind != null)
                            {
                                value = connection.Get<T>(Convert.ToInt32(insert_ind));
                                return true;
                            }
                        }
                    }
